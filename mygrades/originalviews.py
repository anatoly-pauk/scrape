...
@login_required
def standards_curriculum_map(request):
    standard = Standard.objects.count()
    number = Assignment.objects.annotate(Count('standard'))
    standard_count = number[0].standard__count
    hardcount = "10"
    #standard_count = Standard.count()
    my_title = "See a Curriculum Standards Map"
    template_name = "standards_curriculum_map.html"
...
def home_page_view(request):
    ...
    if request.user.groups.filter(name="Student").count() > 0:
        ...
        mystudent = Student.objects.get(email=student.email)
